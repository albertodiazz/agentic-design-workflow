...
function applyFillColor(shape, item) {
  ...
  if (!hasColorLike(shape, color) && typeof shape.setFillColor === "function") {
    try { shape.setFillColor(color); errors = []; } catch (err1) { errors.push(String(err1 && err1.message ? err1.message : err1)); }
  }

  if (!hasColorLike(shape, color) && typeof shape.setFills === "function") {
    try { shape.setFills(fills); errors = []; } catch (err2) { errors.push(String(err2 && err2.message ? err2.message : err2)); }
  }

  var applied = hasColorLike(shape, color);
  return { applied: applied, actual: { has_color: applied }, expected: { fill_color: color }, error: applied ? null : (errors.length ? errors.join("; ") : "fill_color_not_applied") };
}
...
function applyTextColor(shape, item) {
  ...
  if (!hasColorLike(shape, color) && typeof shape.setTextColor === "function") {
    try { shape.setTextColor(color); errors = []; } catch (err1) { errors.push(String(err1 && err1.message ? err1.message : err1)); }
  }

  if (!hasColorLike(shape, color) && typeof shape.setFillColor === "function") {
    try { shape.setFillColor(color); errors = []; } catch (err2) { errors.push(String(err2 && err2.message ? err2.message : err2)); }
  }

  var applied = hasColorLike(shape, color);
  return { applied: applied, actual: { has_color: applied }, expected: { text_color: color }, error: applied ? null : (errors.length ? errors.join("; ") : "text_color_not_applied") };
}
...
function applyStroke(shape, item) {
  ...
  if (typeof hasColorLike === "function" && !hasColorLike(shape, color) && typeof shape.setStrokes === "function") {
    try { shape.setStrokes(strokes); errors = []; } catch (err1) { errors.push(String(err1 && err1.message ? err1.message : err1)); }
  }

  var hasStroke = false;
  try {
    hasStroke = !!shape.strokes || !!shape.strokeColor || !!shape.strokeWidth || (typeof hasColorLike === "function" && hasColorLike(shape, color));
  } catch (err) {
    hasStroke = false;
  }
